...
;~ 		Sleep(2000)
;~ 	WEnd
;~ 	WriteLog("Click $ui_changename_pop_elem3 until $ui_changename_done")
;~ 	ClickPosUntilScreenByMultiPixel($ui_changename_pop_elem3, $ui_changename_done)
	WriteLog("Click top right corner blue setting icon until setting screen appear")
	ClickPosUntilScreenByMultiPixel($menu_setting, $ui_setting)
	WriteLog("Click gear icon until option screen appear")
	ClickPosUntilScreenByMultiPixel($ui_setting_elem3, $ui_option)
	WriteLog("Click account management button until transfer account screen appear")
	ClickPosUntilScreenByMultiPixel($ui_option_elem4, $ui_link)
	WriteLog("Click use email link until email setup screen appear")
	ClickPosUntilScreenByMultiPixel($ui_link_elem1, $ui_email)
	WriteLog("Click register link until terms screen appear")
	ClickPosUntilScreenByMultiPixel($ui_email_elem2, $ui_terms)
	WriteLog("Click agree all")
	ClickOnRelative($ui_terms_elem1)
	Sleep(500)
	WriteLog("Click start button until register form screen appear")
...
;~ 	WaitPixelList($btn_close_announcement)
	...
	ClickPosUntilScreen($menu_maoxian, "btn_skip_tutorial.bmp", "CloseNewYearEventNtf", 1200)
	WriteLog("Click on btn_skip_tutorial.bmp")
	ClickImage("btn_skip_tutorial.bmp")
	Sleep(2000)
	ClickOnRelative($menu_maoxian)
EndFunc   ;==>BackgroundStory

Func Fight1_1()
	WriteLog("1-1 start")
	WriteLog("Click on ready to fight button until ui_ready_to_skip.bmp")
	ClickPosUntilScreen($btn_ready_to_fight_elem1, "ui_ready_to_skip.bmp", Default, 1000)
	SkipConversation()
	ProceeToNextFight()
	WriteLog("你好像很混乱...")
	TrySkipTutorial()
...
Func Fight1_2()
	...
	WriteLog("Click on ready to fight button until ui_ready_to_skip.bmp")
	ClickPosUntilScreen($btn_ready_to_fight_elem1, "ui_ready_to_skip.bmp")
EndFunc   ;==>Fight1_2
