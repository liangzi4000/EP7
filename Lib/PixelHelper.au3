...
Func ClickImageUntilScreenByPixel($waitimage, $untilpixel, $interval = 700, $timeout = 60, $timeoutcall = "", $convertposition = True)
	Local $pos = GetImageCenterPosActive($waitimage)
	ClickPosUntilScreenByPixel($pos, $untilpixel, $interval, $timeout, $timeoutcall, $convertposition)
...
Func ClickPosUntilScreenByMultiPixel($pos, $untilpixellist, $interval = 700, $timeout = 60, $timeoutcall = "", $convertposition = True)
	Local $mypos = $convertposition = True ? ConvertRelativePosToAbsolutePos($pos) : $pos
	Local $x = 0, $y = 0
	Local $hTimer = TimerInit()
	Local $foundcount = 0
...
Func WaitPixelList($pixellist)
	Do
		Sleep(500)
	Until CheckPixelList($pixellist) = True
