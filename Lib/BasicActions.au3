...
Func ClickOnLastPosition()
	...
	Return 1
EndFunc   ;==>ClickOnLastPosition

Func PressAndHold($pos, $second)
	Local $targetpos = ConvertRelativePosToAbsolutePos($pos)
	MouseMove($targetpos[0], $targetpos[1])
	MouseDown($MOUSE_CLICK_LEFT) ; Set the left mouse button state as down.
	Local $holdsec = $second * 1000
	Sleep($holdsec)
	MouseUp($MOUSE_CLICK_LEFT) ; Set the left mouse button state as up.
EndFunc   ;==>PressAndHold

Func DoubleClickOn($pos)
	Local $targetpos = ConvertRelativePosToAbsolutePos($pos)
	MouseClick($MOUSE_CLICK_LEFT, $targetpos[0], $targetpos[1], 2, 0)
	Assign($activewindow & $lastclickposition, $targetpos[0] & "," & $targetpos[1], 2)
	If $debug Then WriteLog("DoubleClickOn [" & $targetpos[0] & "," & $targetpos[1] & "]")
EndFunc   ;==>DoubleClickOn
